<?php
    ...
    $total = $_POST['total'];
        ...
        $sql_cek_stok = "SELECT * FROM keranjang inner join produk on keranjang.id_produk = produk.id_produk where keranjang.id_user=$id";
        ...
        while($data_cek_stok = mysqli_fetch_array($query_cek_stok)){
            if(($data_cek_stok["stok"] - $data_cek_stok["jumlah"]) < 0){
                $nama_produk = $data_cek_stok['nama_produk'];
                header("Location: keranjang.php?status=stok&barang=".$nama_produk);
                return 0;
            }else{
